...
`Production` erhält bei jedem erfolgreichen, zehnsekündigen Status-Poll ein neues `brewingStatus`-Objekt und rendert dadurch vollständig neu. Ein separater lokaler Sekundentakt aktualisiert ausschließlich die wall-clock-basierte Display-Zeit. Dabei ruft `renderInfo()` `buildTemperatureTimelineModel()` erneut auf; echte Temperaturmesspunkte entstehen weiterhin nur aus Controllerstatus-Samples.
...
| Gesamte Production-Ansicht | Neues Statusobjekt sowie der lokale Display-Tick rendern die komplette Class Component samt fachlich unveränderten Settings und Teilen der Wasser-/Prozessanzeige | Wiederholte React-Reconciliation durch den lokalen Sekundentakt; Serverstatus selbst kommt nur etwa alle zehn Sekunden | Hoch | Mit React Profiler auf Zielgerät messen, danach Timeline, Settings und statische Panels an fachlichen Prop-Grenzen als `PureComponent`/`React.memo` trennen; stabile Props sicherstellen. |
