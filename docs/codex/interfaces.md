...
`GET fermentation/beers/{finishedBeerId}/bubble-activity` returns the BeerDataStore-owned technical bubble windows as a chronologically ordered array. Each item uses the wire fields `deviceId`, `sequence`, `bubbleCount`, `windowSeconds`, `averagePressureDeltaPa`, and `windowEndedAt`. `averagePressureDeltaPa` is the average differential pressure in Pa relative to the calibrated sensor baseline; it can be positive, negative, or zero and may be null or absent for legacy rows. Optional inclusive `from` and `to` query parameters are timezone-aware ISO-8601 instants. The UI requests 6-hour, 24-hour, and 7-day ranges with both filters; `Alles` omits both filters.

The UI plots two technical values from the same window in one chart: `bubbleCount * 60 / windowSeconds` Blubbs/min on the left axis and `averagePressureDeltaPa` on the right Pa axis. It does not coerce missing pressure averages to zero, display `deviceId` or `sequence`, infer missing windows, or derive a fermentation assessment. BeerDataStore remains the source of the stored technical history. Deployment of this additive field and exact DTO is **Needs verification** against the BeerDataStore version deployed behind `/api/database`.
...
| PATCH | `fermentation/devices/{encodedDeviceUid}` | Set or reset a user-visible sensor alias | `{ displayName: string | null }`, returns the canonical updated fermentation device; `deviceUid` and `deviceName` remain unchanged |
...
BeerDataStore is the sole persistent source of truth. Brauhaus2 combines `GET fermentation/beers/{finishedBeerId}/recipe-actions` and `GET fermentation/beers/{finishedBeerId}/measurements`; it writes measurements to the matching `/measurements` route and completes/skips actions through `/recipe-actions/{actionId}/complete|skip`. `POST fermentation/beers/{finishedBeerId}/recipe-actions/{actionId}/complete` has an empty JSON body and returns the canonical updated `FermentationAction`; the UI maps that response and replaces only the matching aggregate action immediately. The measurement route is the only read source for beer temperature, ambient temperature, and Plato. Its records use `beerTemperatureC`, `ambientTemperatureC`, `plato`, `source`, optional `sensorId`, and optional `sourceId`; `temperatureC` remains a supported read alias for beer temperature. `source` is display metadata only and never controls editing or rendering. Completion identity is always the pair `FinishedBrew.id` plus `actionId`. The UI does not consume `/sensor-measurements`; `/bubble-activity` is the sensor-specific blubb/pressure history. Their deployed availability is **Needs verification** against the updated BeerDataStore contract.

The direct table navigation and reorganized measurement page do not add or alter an API call. Presentation mappings localize existing `sourceType`, amount-unit, trigger-unit, `completedAt`, and `contactEndsAt` values without changing their wire representations. Chart action markers are restricted to `COMPLETED` actions carrying a parseable backend `completedAt`.
...
`FinishedBrew.fermentationStartedAt` is the nullable, timezone-bearing canonical basis for `TIME_OFFSET`; it is not inferred from `startDate`. The production-completion workflow sends `null`; setting the actual start instant belongs to the separate start-fermentation workflow. Complete PUT payloads preserve the value unchanged.

`POST finishedbeer` additively accepts `fermentationActions`. Each create entry uses `sourceType`, optional `recipeRelationId`, optional `ingredientId`, `name`, `amount`, `unit`, `triggerType`, nullable `triggerValue`, nullable `triggerUnit`, nullable `contactTime`, and nullable `contactTimeUnit`. Production builds these entries from `DRY_HOP` and `FERMENTATION` ingredient rows in the already scaled brew plan; it never scales the amount again. Runtime response entries additionally carry the BeerDataStore-owned `actionId`, status, due/completion/contact projections. Deployment of this extended create DTO remains **Needs verification** against the BeerDataStore version behind `/api/database`.
...
The fermentation measurement UI keeps `deviceUid` as the internal identity for status lookup, assignment, unassignment, and alias updates, but does not display it as user-facing sensor information. The sensor-owned `deviceName` remains read-only. The optional nullable `displayName` is changed only through `PATCH fermentation/devices/{encodedDeviceUid}` with `{displayName}`, and the UI reloads canonical BeerDataStore data after success. It must never call `POST fermentation/devices/register` to rename a device. Deployment of the PATCH route, response, and exact error contract is **Needs verification** with BeerDataStore.

The browser opens a native RFC WebSocket (not Socket.IO) at same-origin `/api/fermentation/ui`, using `ws://` for an HTTP page and `wss://` for an HTTPS page. Caddy owns production routing. The initial `FERMENTATION_GATEWAY_SNAPSHOT` carries `sensors[]`; `FERMENTATION_SENSOR_STATUS_CHANGED` carries one `sensor`. A sensor is keyed by `deviceUid`, has optional `deviceName` and `beerId`, an ISO-8601 `updatedAt`, and one of `REGISTERED | ASSIGNED | UNASSIGNED | DISCONNECTED | BACKEND_UNAVAILABLE | BACKEND_ERROR`. `FERMENTATION_DATA_CHANGED` carries a non-empty `beerId` plus `change: MEASUREMENT | BUBBLE_ACTIVITY | STATE`; it is an invalidation signal only, and the UI reloads canonical BeerDataStore REST data rather than adopting data from the socket. Unknown messages are ignored. For a sensor status carrying `beerId`, the UI uses that explicit ID to target a refresh and does not infer another assignment.
...
## Incremental fermentation measurement contract

`GET fermentation/beers/{finishedBeerId}/measurements?afterId={measurementId}` returns the chronologically ordered suffix strictly after the identified measurement. It has the same DTO as the initial unfiltered read. The fermentation gateway additively publishes `FERMENTATION_MEASUREMENT_RECORDED` with `{beerId, measurement}` and `FERMENTATION_BUBBLE_ACTIVITY_RECORDED` with `{beerId, activity}` after persistence. The UI deduplicates measurements by `measurement.id` and bubble windows by `(deviceId, sequence)`. Deployment and exact not-found behavior for an obsolete cursor are **Needs verification** in BeerDataStore/FermentationSensorGateway.

The UI no longer calls `/sensor-measurements`. Temperature, ambient temperature, and Plato come exclusively from `/measurements`; pressure and bubble windows come from `/bubble-activity`.
