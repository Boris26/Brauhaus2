...
The fermentation measurement UI keeps `deviceUid` as the internal identity for status lookup, assignment, unassignment, and a prepared rename callback, but does not display it as user-facing sensor information. No BeerDataStore rename route is currently documented, so the UI must not call `POST fermentation/devices/register` to rename a device and must not claim persistence when no rename callback is connected. The eventual rename method, route, response, and error contract are **Needs verification** with BeerDataStore.
