...
BeerDataStore remains the persistent source of truth for fermentation devices, assignments, measurements, and recipe actions. FermentationSensorGateway carries live integration/connectivity status plus canonical persisted-record notifications for incremental UI updates; it does not perform UI-side fermentation assessment. REST provides initial state and cursor-based reconnect recovery.
