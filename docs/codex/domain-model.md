...
Fields: `id`, `name`, `startDate`, nullable timezone-bearing `fermentationStartedAt`, optional `fermentationActions`, optional `endDate`, `liters`, `originalwort`, nullable `residual_extract`, `note`, `active`, optional `beer_id`, `state`, optional `brewValues`. `startDate` is historical/calendar metadata and is never an automatic substitute for `fermentationStartedAt`.

`state` values are `WAITING_FOR_FERMENTATION`, `FERMENTATION`, `MATURATION`, and `FINISHED`. `WAITING_FOR_FERMENTATION` means that brewing is complete but fermentation has not actually started. `FERMENTATION` covers the complete time in the fermentation vessel; additions are recipe actions, not global states.

## Ingredients

- `Hops`: `id`, `name`, `type`, `alpha`, `description`.
- Recipe hop entries reference master data by `id`, may contain an optional `name`, and keep brew-day `additionTime`/`timeUnit` separate from optional `DRY_HOP` action fields. Action fields are `actionId`, `triggerType`, `triggerValue`, `triggerUnit`, `contactTime`, and `contactTimeUnit`; non-dry-hop usages contain none of them. BRAUHAUS v2 imports are interpreted by BeerDataStore and returned to this UI in the canonical model. Brauhaus2 has no BRAUHAUS recipe exporter; its PDF exports are presentation documents only.
- `Malts`: `id`, `name`, `description`, `ebc`; recipe malt references master data by `id`, may contain an optional `name`, uses uppercase `EBC`, and has `quantity`.
- `Yeasts`: `id`, `name`, `description`, `temperature`, `type`, `evg`; recipe yeast references master data by `id`, may contain an optional `name`, and uses `EVG`, `temperature`, `type`, and `quantity`.
...
`FinishedBrew` remains the beer/batch identity and status model; no parallel beer entity or status machine is introduced. Unified measurements reference `FinishedBrew.id` and may contain beer temperature, ambient temperature, and Plato. Their `MANUAL | SENSOR` source is provenance only; it does not change measurement semantics or manual-entry availability. Sensor measurements remain a separate model only for sensor-specific values such as blubbs. BeerDataStore owns records and actual timestamps. Brauhaus2 displays them and sends explicit user commands. Sensor availability never implies that fermentation ended. Displayed alcohol and attenuation are approximate and never trigger status changes.

Fermentation presentation labels are UI-only projections: `DRY_HOP` is shown as `Hopfen`, `ADDITIONAL_INGREDIENT` as `Zutat`, and technical amount/time units are localized. Trigger/status enums and timestamps remain unchanged. No additional persisted presentation model or fermentation state is introduced.
