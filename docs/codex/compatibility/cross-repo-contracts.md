...
The Settings UI separately loads persistent agitator defaults from `GET /Settings/Agitator` and sends full `{ speedPercent, runningMinutes, breakMinutes }` replacements to `PUT /Settings/Agitator`. The PUT response is the confirmed source of truth. These values use percent and minutes, cannot contain a negative interval, and cannot set both interval values to zero. This settings contract never calls the runtime `Agitator/Config` endpoint and therefore does not change an active agitator. Deployment of Braumeister #102 with this exact route and response body is **Needs verification** because the control repository was unavailable in this workspace.
