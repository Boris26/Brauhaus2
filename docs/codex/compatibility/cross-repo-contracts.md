...
The Braumeister #106 BrewSession contract is additive: recipe submission includes the persisted `beerId`, `plannedVolume` in liters, and `plannedBrewhouseEfficiency` in percent. After the payload-free `brew-session-running` signal, the UI reads exactly these fields from `GET /BrewSession`; a missing session, unknown beer, or invalid scaling data never issues a controller start command. Deployment of Braumeister #106 remains **Needs verification** against the real multi-client controller.

The production agitator UI loads both `GET /Agitator/Defaults` and `GET /Agitator/Status` once when it initializes. An active detail configuration (`CONTINUOUS` or `AUTOMATIC`) has priority. When the detail mode is `OFF`, its retained configuration values are not considered an active runtime override, so persistent defaults supply `speedPercent`, `runningMinutes`, and `breakMinutes`; the mode remains `OFF`. Production changes still send only full runtime replacements to `PUT /Agitator/Config`, never to `PUT /Agitator/Defaults`, and use `POST /Agitator/Pause` or `POST /Agitator/Resume`. The regular `GET /Status/` response may contain `agitator` runtime fields only or additionally `speedPercent`, `runningMinutes`, and `breakMinutes`. Missing optional configuration fields retain the last confirmed configuration and never cause a control request. Agitator interval timing values are minutes, without UI-side seconds conversion, and speed is a percentage from 0 through 100. The UI labels these values `Geschwindigkeit`, `Laufzeit`, and `Pausenzeit`; the automatic mode toggle is labelled `Intervallbetrieb`. The seconds fields are removed because this UI is deployed with the new controller contract; controller rollout compatibility is **Needs verification**. No permanent detail-status poll is used.

Runtime configuration writes are serialized: while one `PUT /Agitator/Config` is pending, the UI retains only the latest desired full configuration and sends it after the active request completes. HTTP success is only a command acknowledgement. The UI reconciles configuration and pause state exclusively from `agitator-state-changed`; a late bootstrap `GET /Agitator/Status` cannot replace a newer socket snapshot or local draft. This is a UI-side ordering guarantee and does not change the PI/control API shape.

The Settings UI separately loads persistent agitator defaults from `GET /Agitator/Defaults` and sends full `{ speed, intervalOnMinutes, intervalOffMinutes }` replacements to `PUT /Agitator/Defaults`. The PUT response is the confirmed source of truth. Speed is an integer from 0 through 100; both finite interval values use minutes, may be decimal, and must be greater than zero. This settings contract never calls the runtime `Agitator/Config` endpoint and therefore does not change an active agitator. Deployment of Braumeister #102 with this exact route and response body is **Needs verification** because the control repository was unavailable in this workspace. The controller also publishes the complete snapshot as `agitator-defaults-changed` on the already used default Socket.IO namespace `/`; dirty Settings forms retain local input and offer explicit adoption of the external snapshot.
...
BeerDataStore is the source of truth for fermentation measurements, actions, devices, assignments, sensor readings, and completion timestamps. Brauhaus2 is display and interaction only; hardware does not call Brauhaus2. `GET fermentation/beers/{finishedBeerId}/measurements` is the single read contract for `beerTemperatureC`, `ambientTemperatureC`, and manually entered `plato`; `temperatureC` is accepted as a beer-temperature response alias. `source` (`MANUAL | SENSOR`) and optional `sensorId`/`sourceId` are provenance metadata and never gate manual entry. `/sensor-measurements` is not consumed by the UI; `/bubble-activity` remains separate for blubbs/pressure and is not a temperature source for the UI. The contract is keyed by existing `FinishedBrew.id` and does not alter the finished-beer lifecycle. Deployment of this updated DTO and exact errors **Needs verification** with BeerDataStore.

Sensor aliases use the optional nullable `displayName` device field. `deviceUid` remains the stable internal identity and is never rendered or edited; `deviceName` remains sensor-owned and read-only. Brauhaus2 sends `PATCH fermentation/devices/{encodedDeviceUid}` with `{displayName: string | null}`, consumes the returned canonical device, and reloads the beer aggregate rather than updating it optimistically. It never uses `POST fermentation/devices/register` for alias changes. Deployment of this additive DTO field, PATCH route, canonical response, and exact errors is **Needs verification** with BeerDataStore.

The additive technical-history read is `GET fermentation/beers/{finishedBeerId}/bubble-activity`, with optional inclusive timezone-aware ISO-8601 `from` and `to`. Rows retain `deviceId`, `sequence`, `bubbleCount`, `windowSeconds`, `averagePressureDeltaPa`, and `windowEndedAt`; the UI plots count/duration and average differential pressure from that same window against its end time. Blubbs/min is calculated per raw window. `averagePressureDeltaPa` is Pa relative to the calibrated sensor baseline and may be positive, negative, or zero; null/omitted legacy values remain gaps rather than synthetic zeroes. The UI presents no fermentation assessment. BeerDataStore owns ordering and persistence; deployment of the additive pressure field is **Needs verification**.
...
The gateway snapshot may additively include `measurementState: IDLE | RUNNING | PAUSED` per sensor and may publish `FERMENTATION_SENSOR_RUNTIME_CHANGED` with `deviceUid`, `measurementState`, and UTC ISO `updatedAt`. Brauhaus2 stores and displays this transient state only for the sensor assigned to the current beer. Runtime changes do not trigger BeerDataStore REST reloads; a disconnected sensor without `measurementState` has an unknown runtime state rather than inferred `IDLE`.

## Incremental fermentation history (Brauhaus2)

Brauhaus2 now requires the additive exclusive cursor read `GET fermentation/beers/{finishedBeerId}/measurements?afterId={measurementId}` and persisted-record gateway messages `FERMENTATION_MEASUREMENT_RECORDED {beerId, measurement}` and `FERMENTATION_BUBBLE_ACTIVITY_RECORDED {beerId, activity}`. Unfiltered `/measurements` remains the initial-load contract. `/sensor-measurements` is no longer consumed: normal temperatures and Plato belong to canonical `FermentationMeasurements`, while `/bubble-activity` retains sensor-specific count/window/pressure data. BeerDataStore and gateway deployment are **Needs verification**.
