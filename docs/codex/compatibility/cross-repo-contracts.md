...
The additive technical-history read is `GET fermentation/beers/{finishedBeerId}/bubble-activity`, with optional inclusive timezone-aware ISO-8601 `from` and `to`. Rows retain `deviceId`, `sequence`, `bubbleCount`, `windowSeconds`, and `windowEndedAt`; the UI uses only count, duration, and end time for its combined finished-beer chart. Blubbs/min is calculated per raw window, measured zero remains zero, and missing rows remain gaps. BeerDataStore owns ordering and persistence; deployed endpoint compatibility is **Needs verification**.
