...
The production agitator UI loads both `GET /Agitator/Defaults` and `GET /Agitator/Status` once when it initializes. A complete current detail status has priority; persistent defaults supply `speedPercent`, `runningMinutes`, and `breakMinutes` only when no current detail status is available. Production changes still send only full runtime replacements to `PUT /Agitator/Config`, never to `PUT /Agitator/Defaults`, and use `POST /Agitator/Pause` or `POST /Agitator/Resume`. The regular `GET /Status/` response may contain `agitator` runtime fields only or additionally `speedPercent`, `runningMinutes`, and `breakMinutes`. Missing optional configuration fields retain the last confirmed configuration and never cause a control request. Agitator interval timing values are minutes, without UI-side seconds conversion, and speed is a percentage from 0 through 100. The seconds fields are removed because this UI is deployed with the new controller contract; controller rollout compatibility is **Needs verification**. No permanent detail-status poll is used.
