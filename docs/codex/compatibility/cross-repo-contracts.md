...
Sensor rename is UI-prepared only. `deviceUid` remains the stable internal identity and is passed with the trimmed display name through an optional callback, but it is no longer rendered as user-facing sensor information. Brauhaus2 does not use the registration endpoint `POST fermentation/devices/register` for rename and does not introduce a REST action until BeerDataStore provides a dedicated contract. The rename method, path, DTO, canonical response, and errors are **Needs verification** with BeerDataStore.
