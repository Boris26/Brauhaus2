...
- During the return phase after a decoction, `heating.followsDecoction` selects return-specific Production copy. `heating.heaterEnabled` describes whether heating is permitted and drives the user-facing blocked/ready status; it is not a physical switch state. Production flames are driven exclusively by a connected `realtimeState.heatingRunning === true` snapshot; missing or disconnected snapshots are unknown.
