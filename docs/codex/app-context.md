...
The detail view reloads the BeerDataStore fermentation aggregate after saving Plato/temperature measurements. Completion adopts the canonical action returned by BeerDataStore immediately without waiting for polling or another read. Due/pending/completed actions and contact-time status are in-app notifications. Existing PI/control Web Push remains unchanged; delivery of fermentation action/contact-time push events is **Needs verification** in BeerDataStore and the deployment push owner.

The finished-brews table also has a compact per-row measurement action that opens `/finished-brews/{encodedFinishedBeerId}/measurements` directly. The measurement page emphasizes current readings, the combined temperature/Plato chart, one compact latest measurement, and the BeerDataStore-provided fermentation plan. Raw history is collapsed by default. Only completed recipe actions with a concrete `completedAt` are marked on the chart; pending triggers are not projected onto the timeline.
