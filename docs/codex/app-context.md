...
Production completion creates `WAITING_FOR_FERMENTATION`. The fermentation detail view confirms and dispatches the dedicated idempotent BeerDataStore start command, then immediately adopts its canonical `FERMENTATION` response and server-owned `fermentationStartedAt`. The existing normal UI lifecycle remains centrally restricted to `FERMENTATION -> MATURATION | FINISHED` and `MATURATION -> FINISHED`; `FINISHED` is terminal. **Gärung bezeichnet die gesamte Zeit im Gärbehälter. Es gibt keinen regulären globalen Status für Hauptgärung oder Nachgärung. Dry Hop und weitere Zugaben werden über einzelne Rezeptaktionen innerhalb der Gärung gesteuert.** Lifecycle controls live in the fermentation/detail view. `finished-brews` shows status read-only for existing rows; only manual historic creation exposes an explicitly labelled administrative status.

The measurement dashboard reloads the BeerDataStore fermentation aggregate after saving Plato/temperature measurements. Completion adopts the canonical action returned by BeerDataStore immediately without waiting for polling or another read. Due/pending/completed actions and contact-time status are in-app notifications. Existing PI/control Web Push remains unchanged; delivery of fermentation action/contact-time push events is **Needs verification** in BeerDataStore and the deployment push owner.

Manual fermentation values are entered through one reusable dialog shared by the measurement dashboard and the finished-brew table quick action. Every opening initializes a fresh local date/time value. The UI offers and accepts this action only while the latest finished-brew state is `FERMENTATION`; it checks that state again on submit, so an already open dialog cannot save after a transition to `MATURATION` or `FINISHED`. The existing measurement endpoint and DTO are unchanged, and sensor availability or assignment is deliberately not part of this permission check.

The eye action in the finished-brews table opens `/finished-brews/{encodedFinishedBeerId}/measurements` directly. This measurement page is the single finished-brew dashboard; no separate table-overlay detail view is maintained. The measurement page emphasizes current readings, the combined temperature/Plato chart, one compact latest measurement, and the BeerDataStore-provided fermentation plan. Raw history is collapsed by default. Only completed recipe actions with a concrete `completedAt` are marked on the chart; pending triggers are not projected onto the timeline.

Production sends `FinishedBrew.fermentationStartedAt: null` when the completed brewing workflow creates the waiting record. The same request contains concrete, already scaled `fermentationActions`, and retries reuse the same payload. Detail displays never substitute `startDate`; records without the canonical timestamp show no inferred fermentation day.
