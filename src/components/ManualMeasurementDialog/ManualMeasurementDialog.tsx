...
export const ManualMeasurementDialogView: React.FC<Props> = ({open, beerId, onClose, brewState, saving, error, saveMeasurement}) => {
  ...
  return <AppDialog open={open} onClose={onClose} disableClose={saving} title="Manuelle Gärungsmessung" maxWidth="xs" className="manual-measurement-dialog" icon={<ScienceOutlinedIcon/>}
    actions={<><DialogCancelButton onClick={onClose} disabled={saving}/><Button className="brauhaus-button brauhaus-button-primary" color="primary" variant="contained" onClick={submit} disabled={saving || unavailable} startIcon={!saving ? <SaveOutlinedIcon/> : undefined}>{saving ? <><CircularProgress size={18} sx={{mr: 1}}/>Speichert …</> : 'Speichern'}</Button></>}>
