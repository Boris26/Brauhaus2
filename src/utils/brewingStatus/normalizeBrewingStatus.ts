...
export const normalizeBrewingStatus = (aRawStatus: unknown): BrewingStatus => {
 ...
 return { elapsedTime: Number(aRaw.elapsedTime ?? aLegacy.elapsedTime ?? 0), process: { state: normalizeState(aRaw.process?.state) }, currentStep: { index: typeof aRaw.currentStep?.index === 'number' ? aRaw.currentStep.index : aLegacy.index, count: typeof aRaw.currentStep?.count === 'number' ? aRaw.currentStep.count : undefined, phase: aStructuredPhase !== ProcessPhase.NONE ? aStructuredPhase : aLegacyPhase, mode: (() => { const aMode = normalizeMode(aRaw.currentStep?.mode); return aMode !== ProcessMode.NONE ? aMode : aLegacyMode; })(), name: aRaw.currentStep?.name ?? aLegacy.Name, duration: aRaw.currentStep?.duration, elapsedTime: aRaw.currentStep?.elapsedTime, remainingTime: aRaw.currentStep?.remainingTime, type: aRaw.currentStep?.type ?? aLegacy.Type }, temperature: { current: aRaw.temperature?.current ?? aLegacy.Temperature, target: aRaw.temperature?.target ?? aLegacy.TargetTemperature }, heating: isObject(aRaw.heating) ? { followsDecoction: typeof aRaw.heating.followsDecoction === 'boolean' ? aRaw.heating.followsDecoction : undefined } : undefined, waiting: { waitingFor: normalizeWaitingFor(aRaw.waiting?.waitingFor), canConfirm: typeof aRaw.waiting?.canConfirm === 'boolean' ? aRaw.waiting.canConfirm : !!aLegacy.WaitingStatus }, error: { code: aRaw.error?.code ?? null, details: aRaw.error?.details ?? null } };
};
