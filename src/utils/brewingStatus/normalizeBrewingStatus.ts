import {Alarm, BrewingStatus, LegacyBrewingStatus, ProcessMode, ProcessPhase, ProcessState, WaitingFor, WaitingState} from '../../model/brewingStatus.types';
import {AgitatorRuntimeStatus} from '../../model/Agitator';
const isObject = (aValue: unknown): aValue is Record<string, any> => typeof aValue === 'object' && aValue !== null;
const isAgitatorRuntimeStatus = (aValue: unknown): aValue is AgitatorRuntimeStatus => {
 const aStatus = isObject(aValue) ? aValue : {};
 return (aStatus.mode === 'OFF' || aStatus.mode === 'CONTINUOUS' || aStatus.mode === 'AUTOMATIC')
     && typeof aStatus.paused === 'boolean'
     && (aStatus.operation === 'OFF' || aStatus.operation === 'CONTINUOUS' || aStatus.operation === 'INTERVAL')
     && typeof aStatus.actualOutputOn === 'boolean';
};
...
const normalizeAlarms = (aValue: unknown): Alarm[] => Array.isArray(aValue)
    ? aValue as Alarm[]
    : [];
...
export const normalizeBrewingStatus = (aRawStatus: unknown): BrewingStatus => {
 ...
 return { elapsedTime: Number(aRaw.elapsedTime ?? aLegacy.elapsedTime ?? 0), currentTime: Number(aRaw.currentTime ?? aLegacy.currentTime ?? 0), agitator: isAgitatorRuntimeStatus(aRaw.agitator) ? aRaw.agitator : undefined, process: { state: normalizeState(aRaw.process?.state) }, currentStep: { index: typeof aRaw.currentStep?.index === 'number' ? aRaw.currentStep.index : aLegacy.index, count: typeof aRaw.currentStep?.count === 'number' ? aRaw.currentStep.count : undefined, phase: aStructuredPhase !== ProcessPhase.NONE ? aStructuredPhase : aLegacyPhase, mode: (() => { const aMode = normalizeMode(aRaw.currentStep?.mode); return aMode !== ProcessMode.NONE ? aMode : aLegacyMode; })(), name: aRaw.currentStep?.name ?? aLegacy.Name, duration: aRaw.currentStep?.duration, elapsedTime: aRaw.currentStep?.elapsedTime, remainingTime: aRaw.currentStep?.remainingTime, type: aRaw.currentStep?.type ?? aLegacy.Type }, temperature: { current: aRaw.temperature?.current ?? aLegacy.Temperature, target: aRaw.temperature?.target ?? aLegacy.TargetTemperature }, hardware: { heater: aHeater, agitator: aAgitator }, heating: isObject(aRaw.heating) ? { followsDecoction: typeof aRaw.heating.followsDecoction === 'boolean' ? aRaw.heating.followsDecoction : undefined, heaterEnabled: typeof aRaw.heating.heaterEnabled === 'boolean' ? aRaw.heating.heaterEnabled : undefined } : undefined, waiting: { waitingFor: normalizeWaitingFor(aRaw.waiting?.waitingFor), canConfirm: typeof aRaw.waiting?.canConfirm === 'boolean' ? aRaw.waiting.canConfirm : !!aLegacy.WaitingStatus }, error: { code: aRaw.error?.code ?? null, details: aRaw.error?.details ?? null }, alarms: normalizeAlarms(aRaw.alarms) };
};
