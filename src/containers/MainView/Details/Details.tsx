...
export class Details extends React.Component<DetailsProps, DetailsState> {
    ...
    renderBatchSettings() {
        ...
        return <section className="recipe-settings" aria-label="Rezeptskalierung">
    ...
    renderGeneralData() {
        ...
        return <section className="recipe-section"><h2>Allgemeine Daten</h2><dl className="recipe-data-grid">
    ...
    renderMashing() {
        ...
        return <section className="recipe-section"><h2>Maischplan</h2>{steps.length ? <ol className="process-list">{steps.map((step, index) =>
            <li key={step.stepId || index} className="process-step"><span className="process-marker" aria-hidden="true">{index + 1}</span><div>
                <strong>{this.mashingStepLabel(step)}</strong><div className="process-values">
                    {step.temperature !== undefined && <span>{step.temperature} °C</span>}{step.time !== undefined && <span>{step.time} min</span>}
    ...
    renderMalts() {
        ...
        return <section className="recipe-section"><h2>Schüttung</h2>{malts.length ? <ul className="ingredient-list">{malts.map((malt, index) =>
    ...
    renderBoiling() {
        ...
        return <section className="recipe-section"><h2>Würzekochen</h2><dl className="recipe-data-grid recipe-data-grid--compact">
        ...
        </dl><h3>Hopfengaben</h3>{hops.length ? <ul className="ingredient-list">{hops.map((hop, index) => <li key={`${hop.id}-${index}`}>
            <strong>{this.ingredientName(this.props.hops, hop.id, 'Zutat', hop.name)}</strong><span>{hop.quantity} g · {hop.additionTime ?? '—'} min · {hop.usage}</span>
        </li>)}</ul> : <p className="recipe-empty">Keine Hopfengaben hinterlegt.</p>}</section>;
    }
    ...
    renderFermentationMaturation() {
        ...
        return <section className="recipe-section"><h2>Gärung &amp; Reifung</h2><dl className="recipe-data-grid recipe-data-grid--compact">
            <div className="recipe-data-item"><dt>Gärtemperatur</dt><dd>{fm.fermentationTemperature ?? '—'} °C</dd></div><div className="recipe-data-item"><dt>Karbonisierung</dt><dd>{fm.carbonation ?? '—'}</dd></div>
        </dl><h3>Hefe</h3>{yeasts.length ? <ul className="ingredient-list">{yeasts.map((yeast, index) => <li key={`${yeast.id}-${index}`}>
            <strong>{this.ingredientName(this.props.yeasts, yeast.id, 'Hefe', yeast.name)}</strong><span>{yeast.quantity}</span>
        </li>)}</ul> : <p className="recipe-empty">Keine Hefe hinterlegt.</p>}</section>;
    }

    renderWater() {
        const beer = this.props.selectedBeer;
        if (!beer) return null;
        return <section className="recipe-section"><h2>Wasser</h2><dl className="recipe-data-grid recipe-data-grid--compact">
