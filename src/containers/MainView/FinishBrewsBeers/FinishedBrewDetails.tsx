...
interface Props { brew: FinishedBrew; details?: FermentationDetails; activeBrews: FinishedBrew[]; loading: boolean; saving: boolean; savingLifecycle: boolean; completing: string[]; assigning: string[]; error?: string; lifecycleError?: string; viewMode?: 'dashboard' | 'measurements'; load: (id: string) => void; save: (value: CreateFermentationMeasurement) => void; complete: (brewId: string, actionId: string) => void; transition: (brew: FinishedBrew) => void; assign: (deviceId: string, brewId: string) => void; openMeasurements?: (id: string) => void; closeMeasurements?: () => void; }
...
const ActionItem: React.FC<{action: FermentationAction; completing: string[]; complete: () => void}> = ({action, completing, complete}) => {
  const contact = contactStatus(action);
  return <li><time>{action.state === 'COMPLETED' ? `Erledigt ${date(action.completedAt || undefined)}` : actionDueLabel(action).label}</time><strong>{actionText(action)}</strong><span>Zugabe: {actionTriggerLabel(action)}</span>{contactTimeLabel(action) && <span>Kontaktzeit: {contactTimeLabel(action)}</span>}{contact === 'running' && <span>Kontaktzeit läuft{action.contactEndsAt ? ` · endet ${date(action.contactEndsAt)}` : ''}</span>}{contact === 'ended' && <span className="fermentation-error">Kontaktzeit beendet{action.contactEndsAt ? ` · ${date(action.contactEndsAt)}` : ''}</span>}{canCompleteAction(action) && <button disabled={completing.includes(action.id)} onClick={complete}>{completing.includes(action.id) ? 'Wird gespeichert …' : 'Als ausgeführt bestätigen'}</button>}</li>;
};
...
export const FinishedBrewDetailsView: React.FC<Props> = props => {
  ...
  const nextAction = [...details.actions].filter(action => action.state === 'PENDING').sort((a, b) => Number(isActionDue(b)) - Number(isActionDue(a)) || Date.parse(a.scheduledAt || '') - Date.parse(b.scheduledAt || ''))[0];
  const dueActions = details.actions.filter(action => action.state === 'PENDING' && (isActionDue(action) || action.triggerType === FermentationTriggerType.MANUAL));
  const pendingActions = details.actions.filter(action => !dueActions.includes(action) && action.state !== 'COMPLETED' && action.state !== 'SKIPPED');
  const completedActions = details.actions.filter(action => action.state === 'COMPLETED');
  ...
  if (props.viewMode === 'measurements') return <div className="finished-brew-details fermentation-details fermentation-measurements-page">
    ...
    <div className="fermentation-measurements-grid"><section className="fermentation-card"><h4>Messverlauf</h4>{measurements.length === 0 ? <p>Keine Messwerte vorhanden.</p> : <ul className="fermentation-list">{measurements.map(measurement => <li key={measurement.id}><time>{date(measurement.measuredAt)}</time><strong>{[number(measurement.temperature, ' °C'), number(measurement.plato, ' °P')].join(' · ')}</strong>{measurement.note && <span>{measurement.note}</span>}</li>)}</ul>}</section>
      ...
      <section className="fermentation-card"><h4>Gärungsaktionen</h4>{details.actions.length === 0 ? <p>Keine Aktionen vorhanden.</p> : <>{dueActions.length > 0 && <><h5>Fällig</h5><ul className="fermentation-list">{dueActions.map(action => <ActionItem key={action.id} action={action} completing={props.completing} complete={() => props.complete(props.brew.id, action.id)} />)}</ul></>}{pendingActions.length > 0 && <><h5>Anstehend</h5><ul className="fermentation-list">{pendingActions.map(action => <ActionItem key={action.id} action={action} completing={props.completing} complete={() => props.complete(props.brew.id, action.id)} />)}</ul></>}{completedActions.length > 0 && <><h5>Erledigt</h5><ul className="fermentation-list">{completedActions.map(action => <ActionItem key={action.id} action={action} completing={props.completing} complete={() => props.complete(props.brew.id, action.id)} />)}</ul></>}</>}</section></div>
