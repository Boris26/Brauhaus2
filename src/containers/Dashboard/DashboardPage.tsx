...
export class DashboardPage extends React.Component<DashboardPageProps, DashboardPageState> {
  ...
  render(): React.ReactNode {
    ...
    const activeBreakdown = [kpis.fermentationCount ? `${kpis.fermentationCount} in Gärung` : '', kpis.maturationCount ? `${kpis.maturationCount} in Reifung` : ''].filter(Boolean).join(' · ') || 'Keine aktiven Biere';
    const cards = [
    ...
    return <main className="dashboard-page" aria-labelledby="dashboard-title">
      <PageHeader icon={<DashboardOutlinedIcon />} title={<span id="dashboard-title">Dashboard</span>} subtitle="Brauhaus auf einen Blick" actions={this.props.isFetching ? <span className="dashboard-loading">Dashboard-Daten werden geladen …</span> : undefined} />
      ...
      <div className="dashboard-content-grid">
        <section className="dashboard-card dashboard-main-card dashboard-active-card" aria-labelledby="dashboard-active-title"><div className="dashboard-section-header"><LocalDrinkIcon aria-hidden="true" /><h2 id="dashboard-active-title">Aktive Gärungen</h2></div>{activeRows.length === 0 ? <p className="dashboard-empty">Keine Biere in Gärung oder Reifung.</p> : <div className="dashboard-scroll-list">{activeRows.map((row) => { const detail = this.props.fermentationByBrewId?.[row.id]; const readings = latestFermentationReadings(detail?.measurements); const next = [...(row.fermentationActions ?? detail?.actions ?? [])].filter(a => a.status === 'PENDING').sort((a,b) => Number(b.due === true) - Number(a.due === true))[0]; const due = next && actionDueLabel(next); return <article className="dashboard-active-row" key={row.id} role="button" tabIndex={0} aria-label={`Messdaten für ${row.name} öffnen`} onClick={() => this.props.openMeasurements?.(row.id)} onKeyDown={event => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); this.props.openMeasurements?.(row.id); } }}><span className={`dashboard-state-dot ${row.state === eBrewState.MATURATION ? 'is-maturation' : ''}`} /><div className="dashboard-active-details"><h3>{row.name}</h3><dl className="dashboard-fermentation-values"><div><dt>Bier</dt><dd>{formatFermentationValue(readings.beerTemperature, '°C')}</dd></div><div><dt>Außen</dt><dd>{formatFermentationValue(readings.ambientTemperature, '°C')}</dd></div><div><dt>Plato</dt><dd>{formatFermentationValue(readings.plato, '°P')}</dd></div></dl>{next && <p className={`dashboard-next-action is-${due?.severity}`}><strong>{due?.label} · {actionTriggerLabel(next)}:</strong> {[next.amount, next.unit, next.name, next.sourceType?.toLowerCase()].filter(Boolean).join(' ')}</p>}</div><span className={`dashboard-badge ${row.state === eBrewState.MATURATION ? 'is-maturation' : ''}`}>{row.stateLabel}</span><strong className="dashboard-day">{row.daysSinceStartLabel === '-' ? '–' : `Tag ${row.daysSinceStartLabel.replace(' Tage', '')}`}</strong></article>;})}</div>}</section>
