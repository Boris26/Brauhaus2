...
import {createProcessSteps, ProcessListCurrentStep, getActiveProcessStepIndex, ProcessList} from './ProcessList';
...
const makeCurrentStep = (index: number, mode: ProcessMode, name: string): ProcessListCurrentStep => ({
    index,
...
const renderProcessList = (currentStep: ProcessListCurrentStep) => (
    <ProcessList
        selectedBeer={selectedBeer}
        currentStepIndex={currentStep.index ?? 0}
        currentStepPhase={currentStep.phase}
        currentStepMode={currentStep.mode}
        currentStepName={currentStep.name}
    />
);

const getActiveStepText = (currentStep: ProcessListCurrentStep) => {
    const {container} = render(renderProcessList(currentStep));
...
describe('ProcessList current-step mapping', () => {
    ...
    it('updates the active marker when the reported control step changes', () => {
        const {container, rerender} = render(renderProcessList(makeCurrentStep(2, ProcessMode.TIMER_RUNNING, 'Rast 1')));
        expect(container.querySelector('li.active')).toHaveTextContent('Rast 1');
        expect(getActiveItems(container)).toHaveLength(1);

        rerender(renderProcessList(makeCurrentStep(2, ProcessMode.TIMER_RUNNING, 'Rast 1')));
        expect(container.querySelector('li.active')).toHaveTextContent('Rast 1');

        rerender(renderProcessList(makeCurrentStep(3, ProcessMode.TIMER_RUNNING, 'Rast 2')));
        expect(container.querySelector('li.active')).toHaveTextContent('Rast 2');

        rerender(renderProcessList(makeCurrentStep(4, ProcessMode.HEATING, 'Rast 3')));
        ...
        rerender(renderProcessList(makeCurrentStep(4, ProcessMode.TIMER_RUNNING, 'Rast 3')));
    ...
    it('moves the active marker when only the mode changes and the control index stays the same', () => {
        const {container, rerender} = render(renderProcessList(makeCurrentStep(4, ProcessMode.HEATING, 'Rast 3')));
        expect(container.querySelector('li.active')).toHaveTextContent('Aufheizen für Rast 3');
        expect(getActiveItems(container)).toHaveLength(1);
    ...
    it('renders all expected rows', () => {
        render(renderProcessList({index: 1, phase: ProcessPhase.MASHING_IN, mode: ProcessMode.HEATING, name: 'Einmaischen'}));
