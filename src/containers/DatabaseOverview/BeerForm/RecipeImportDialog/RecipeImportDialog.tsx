...
export const RecipeImportDialog: React.FC<RecipeImportDialogProps> = ({open, loading = false, backendError, result, masterData = {MALT: [], HOP: [], YEAST: [], ADDITIONAL_INGREDIENT: []}, onCancel, onImport, onCreateMasterData}) => {
    ...
    return <AppDialog open={open} onClose={resetAndCancel} disableClose={busy} title={unresolved.length ? 'Zutaten zuordnen' : 'Rezept importieren'} variant={backendError ? 'error' : busy ? 'progress' : 'info'} className="recipe-import-dialog"
        ...
        </div> : <div className="brauhaus-form"><label className="brauhaus-form-field recipe-import-dialog__format"><span className="brauhaus-form-field__label">Importformat</span><select className="brauhaus-form-control" value={format} disabled={loading} onChange={event => { setFormat(event.target.value as RecipeImportFormat); if (recipe) setIdempotencyKey(createImportIdempotencyKey()); }}><option value="">Bitte auswählen</option><option value={RecipeImportFormat.BRAUHAUS}>Brauhaus</option><option value={RecipeImportFormat.MMUM}>MaischeMalzundMehr (MMuM)</option></select></label><Button className="recipe-import-dialog__file-button brauhaus-button brauhaus-button-secondary" component="label" variant="outlined" fullWidth disabled={loading}>{fileName || 'Datei auswählen'}<input hidden type="file" accept=".json,application/json" onChange={event => { void readFile(event.target.files?.[0]); event.target.value = ''; }}/></Button></div>}
