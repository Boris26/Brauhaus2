...
export const RecipeImportDialog: React.FC<RecipeImportDialogProps> = ({open, loading = false, backendError, result, masterData = {MALT: [], HOP: [], YEAST: [], ADDITIONAL_INGREDIENT: []}, onCancel, onImport, onCreateMasterData}) => {
    ...
    const createField = (name: string, label: string, type: 'text' | 'number' = 'text') => <FormField label={label} type={type} value={createValues[name] ?? ''} onChange={event => setCreateValues(values => ({...values, [name]: type === 'number' ? Number(event.target.value) : event.target.value}))}/>;

    return <AppDialog open={open} onClose={resetAndCancel} disableClose={busy} title={unresolved.length ? 'Zutaten zuordnen' : 'Rezept importieren'} variant={backendError ? 'error' : busy ? 'progress' : 'info'} className="recipe-import-dialog" icon={<FileUploadOutlinedIcon/>}
        actions={<><DialogCancelButton onClick={resetAndCancel} disabled={busy}/>
            {unresolved.length ? <Button className="recipe-import-dialog__import-button brauhaus-button brauhaus-button-primary" onClick={retry} color="primary" variant="contained" disabled={!resolutionComplete || busy} startIcon={!busy ? <CheckCircleOutlineRoundedIcon/> : undefined}>{busy ? <><CircularProgress size={18}/>&nbsp;Import abschließen…</> : 'Import abschließen'}</Button>
            : <Button className="recipe-import-dialog__import-button brauhaus-button brauhaus-button-primary" onClick={() => { if (format && recipe && idempotencyKey && !submitted) { setSubmitted(true); onImport({format, recipe, idempotencyKey}); } }} color="primary" variant="contained" disabled={!format || !recipe || !idempotencyKey || !!parseError || busy} startIcon={!busy ? <FileUploadOutlinedIcon/> : undefined}>{busy ? <><CircularProgress size={18}/>&nbsp;Importieren…</> : 'Importieren'}</Button>}</>}>
        {unresolved.length ? <div className="recipe-import-dialog__resolutions" aria-label="Nicht eindeutig zugeordnete Zutaten">
            ...
            {unresolved.map(item => { const key = keyFor(item); const options = resolutionOptions(item, masterData); return <section className="recipe-import-dialog__resolution" key={key}>
                ...
                <div className="recipe-import-dialog__mapping"><label className="brauhaus-form-field"><span className="brauhaus-form-field__label">Lokale Zuordnung</span><select className="brauhaus-form-control" value={isValidIngredientId(mappings[key]) ? String(mappings[key]) : ''} onChange={event => { if (isValidIngredientId(event.target.value)) { setMappings(current => ({...current, [key]: event.target.value})); setMappingStates(current => ({...current, [key]: 'manual'})); } }}><option value="">Bitte auswählen</option>{options.map(option => <option key={String(option.id)} value={String(option.id)}>{option.name}{option.matchType === 'FUZZY' ? ' (ähnlich)' : ''}</option>)}</select></label>
                ...
                {creating === key ? <div className="recipe-import-dialog__create">{createField('name', 'Name')}{createField('description', 'Beschreibung')}{item.ingredientType === 'MALT' && createField('ebc', 'EBC', 'number')} {item.ingredientType === 'HOP' && <>{createField('type', 'Typ')}{createField('alpha', 'Alpha (%)', 'number')}</>} {item.ingredientType === 'YEAST' && <>{createField('type', 'Typ')}{createField('evg', 'EVG (%)', 'number')}{createField('temperature', 'Temperatur (°C)', 'number')}</>}<Button className="brauhaus-button brauhaus-button-primary" onClick={() => void create(item)} disabled={!String(createValues.name ?? '').trim()}>Anlegen und zuordnen</Button>{createError && <p role="alert" className="recipe-import-dialog__error">{createError}</p>}</div> : <Button className="recipe-import-dialog__create-button brauhaus-button brauhaus-button-secondary" onClick={() => startCreate(item)}>Neue Zutat anlegen</Button>}
            </section>; })}
        </div> : <div className="brauhaus-form"><label className="brauhaus-form-field recipe-import-dialog__format"><span className="brauhaus-form-field__label">Importformat</span><select className="brauhaus-form-control" value={format} disabled={loading} onChange={event => { setFormat(event.target.value as RecipeImportFormat); if (recipe) setIdempotencyKey(createImportIdempotencyKey()); }}><option value="">Bitte auswählen</option><option value={RecipeImportFormat.BRAUHAUS}>Brauhaus</option><option value={RecipeImportFormat.MMUM}>MaischeMalzundMehr (MMuM)</option></select></label><Button className="recipe-import-dialog__file-button brauhaus-button brauhaus-button-secondary" component="label" variant="outlined" fullWidth disabled={loading} startIcon={<FolderOpenOutlinedIcon/>}>{fileName || 'Datei auswählen'}<input hidden type="file" accept=".json,application/json" onChange={event => { void readFile(event.target.files?.[0]); event.target.value = ''; }}/></Button></div>}
