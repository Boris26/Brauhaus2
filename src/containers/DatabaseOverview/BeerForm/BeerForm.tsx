...
export class BeerForm extends React.Component<BeerFormProps, BeerFormState> {
    ...
    renderCreateBeerForm() {
        ...
        const maltsContent = (
            <>
                {maltsDTO.length > 0 && <div className="compact-column-header ingredient-quantity-grid" aria-hidden="true"><span>Malz</span><span>Menge</span><span /></div>}
                <div className="recipe-item-list">{maltsDTO?.map((step, index) => <article className="recipe-item-card compact-row-card ingredient-quantity-grid" key={index}>
                    <label className="compact-field"><span>Malz</span><select aria-label={`Malz ${index + 1}`} name="id" value={String(step.id)} onChange={(e) => this.handleMaltChange(e.target.value, e.target.name, index)} required={true}><option value="">Malz</option>{!malts.some(malt => String(malt.id) === String(step.id)) && step.id !== '' && <option value={String(step.id)}>Unbekanntes Malz (ID {step.id})</option>}{malts.map((malt) => <option key={malt.id} value={String(malt.id)}>{malt.name}</option>)}</select></label><label className="compact-field compact-quantity"><span>Menge</span><span className="quantity-with-unit"><input aria-label={`Malzmenge ${index + 1}`} type="number" name="quantity" min={0} value={step.quantity} onChange={(e) => this.handleMaltChange(e.target.value, e.target.name, index)} required={true} /><em>g</em></span>{this.renderFieldError(`maltsDTO.${index}.quantity`)}</label><button type="button" className="cancel-btn brauhaus-button brauhaus-button-danger brauhaus-icon-button" onClick={() => this.removeMalts(index)} title="Malz löschen" aria-label="Malz löschen"><DeleteOutlineIcon fontSize="small" /></button>
                </article>)}</div>
        ...
        const hopsContent = (
            <>
                {hopsDTO.length > 0 && <div className="compact-column-header hop-row-grid" aria-hidden="true"><span>Hopfen</span><span>Menge</span><span>Verwendung</span><span>Zeit</span><span>Einheit</span><span /></div>}
                <div className="hop-card-list">{hopsDTO?.map((step, index) => <article className={`hop-card compact-row-card ${step.usage === HopUsage.DRY_HOP ? 'is-expanded is-dry-hop' : ''}`} key={index}>
                    ...
                    </div><button type="button" className="cancel-btn brauhaus-button brauhaus-button-danger brauhaus-icon-button" onClick={() => this.removeHops(index)} title="Hopfen löschen" aria-label="Hopfen löschen"><DeleteOutlineIcon fontSize="small" /></button>
        ...
        const yeastContent = (
            <>
                {yeastsDTO.length > 0 && <div className="compact-column-header ingredient-quantity-grid" aria-hidden="true"><span>Hefe</span><span>Menge</span><span /></div>}
                <div className="recipe-item-list">{yeastsDTO?.map((step, index) => <article className="recipe-item-card compact-row-card ingredient-quantity-grid" key={index}>
                    <label className="compact-field"><span>Hefe</span><select aria-label={`Hefe ${index + 1}`} name="id" value={String(step.id)} onChange={(e) => this.handleYeastChange(e.target.value, e.target.name, index)} required={true}><option value="">Hefe</option>{!yeasts.some(yeast => String(yeast.id) === String(step.id)) && step.id !== '' && <option value={String(step.id)}>Unbekannte Hefe (ID {step.id})</option>}{yeasts.map((yeast) => <option key={yeast.id} value={String(yeast.id)}>{yeast.name}</option>)}</select></label><label className="compact-field compact-quantity"><span>Menge</span><span className="quantity-with-unit"><input aria-label={`Hefemenge ${index + 1}`} type="number" name="quantity" min={0} step="0.1" value={step.quantity} onChange={(e) => this.handleYeastChange(e.target.value, e.target.name, index)} required={true} /><em>g</em></span>{this.renderFieldError(`yeastsDTO.${index}.quantity`)}</label><button type="button" className="cancel-btn brauhaus-button brauhaus-button-danger brauhaus-icon-button" onClick={() => this.removeYeast(index)} title="Hefe löschen" aria-label="Hefe löschen"><DeleteOutlineIcon fontSize="small" /></button>
                </article>)}</div>
        ...
        const additionalContent = (
            <>
                {additionalIngredientsDTO.length > 0 && <div className="compact-column-header additional-row-grid" aria-hidden="true"><span>Zutat</span><span>Menge</span><span>Einheit</span><span>Phase</span><span>Zeit</span><span /></div>}
                <div className="recipe-item-list">{additionalIngredientsDTO?.map((aStep, aIndex) => <article className={`recipe-item-card compact-row-card additional-ingredient-card ${aStep.phase === AdditionalIngredientPhase.FERMENTATION ? 'is-expanded' : ''}`} key={aIndex}>
                    <div className="recipe-item-fields additional-ingredient-base">
                        ...
                        {aStep.phase !== AdditionalIngredientPhase.FERMENTATION && <label>Zeit<span className="addition-time-controls"><input aria-label="Zugabezeit" type="number" min={1} name="additionTime" value={aStep.additionTime ?? ''} onChange={(e) => this.handleAdditionalIngredientChange(e.target.value, "additionTime", aIndex)} /><select aria-label="Zeiteinheit" name="timeUnit" value={aStep.timeUnit ?? AdditionalIngredientTimeUnit.DAYS} onChange={(e) => this.handleAdditionalIngredientChange(e.target.value, "timeUnit", aIndex)}><option value={AdditionalIngredientTimeUnit.MINUTES}>min</option><option value={AdditionalIngredientTimeUnit.HOURS}>Std.</option><option value={AdditionalIngredientTimeUnit.DAYS}>Tage</option></select></span></label>}
