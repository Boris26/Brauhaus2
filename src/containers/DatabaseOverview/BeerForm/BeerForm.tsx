...
import {AdditionalIngredientPhase, AdditionalIngredientTimeUnit, Beer, FermentationSteps} from "../../../model/Beer";
...
import {IngredientType, RecipeImportRequest, RecipeImportResult} from '../../../model/RecipeImport';
import {RecipeImportDialog} from './RecipeImportDialog/RecipeImportDialog';
...
export class BeerForm extends React.Component<BeerFormProps, BeerFormState> {
    ...
    constructor(props: BeerFormProps) {
        ...
        const defaultState = {
            ...
            maltsDTO: [{ id: '', quantity: undefined as any }],
            hopsDTO: [{ id: '', quantity: undefined as any, usage: HopUsage.BOIL }],
            yeastsDTO: [{ id: '', quantity: undefined as any }],
        ...
        const saved = props.beerFormState || {};
        this.state = {
            ...defaultState,
            ...saved,
            hopsDTO: (saved.hopsDTO ?? defaultState.hopsDTO).map((hop: HopDTO) => normalizeHopDto(hop)),
            additionalIngredientsDTO: (saved.additionalIngredientsDTO ?? defaultState.additionalIngredientsDTO).map((ingredient: AdditionalIngredientDTO) => this.normalizeAdditionalIngredient(ingredient)),
            fermentationSteps,
            validationErrors: {
    ...
    createImportMasterData = async (type: IngredientType, values: Record<string, string | number>) => {
        if (type === 'MALT') {
            const created = await MaltRepository.submitMalt(values as any); this.props.getMalt(true); return created;
        }
        if (type === 'HOP') {
            const created = await HopRepository.submitHop(values as any); this.props.getHop(true); return created;
        }
        if (type === 'YEAST') {
            const created = await YeastRepository.submitYeast(values as any); this.props.getYeast(true); return created;
        }
        const created = await AdditionalIngredientRepository.submitAdditionalIngredient(values as any);
    ...
    componentDidUpdate(prevProps: Readonly<BeerFormProps>, prevState: Readonly<BeerFormState>, snapshot?: any) {
        ...
        if (importedBeer && importedBeer !== prevProps.importedBeer) {
            this.setState({
                ...
                maltsDTO: importedBeer.malts ? importedBeer.malts.map(m => ({ id: m.id, quantity: m.quantity })) : [],
                ...
                yeastsDTO: importedBeer.fermentationMaturation && importedBeer.fermentationMaturation.yeast ? importedBeer.fermentationMaturation.yeast.map(y => ({ id: y.id, quantity: y.quantity })) : [],
    ...
    handleHopChange = (aValue: string, aName: string, aIndex: number) => {
        this.setState((prevState) => {
            ...
            if (aName === "usage") {
                ...
                hopsDTO[aIndex] = aValue === '' ? clearRecipeAction(step) : normalizeRecipeAction({...step, triggerType: aValue as TriggerType});
    ...
    normalizeAdditionalIngredient = (aIngredient: AdditionalIngredientDTO): AdditionalIngredientDTO => {
        ...
        const normalized: AdditionalIngredientDTO = {
            ...
            phase,
            additionTime: phase === AdditionalIngredientPhase.FERMENTATION ? undefined : (aIngredient.additionTime == null ? undefined : Number(aIngredient.additionTime)),
            ...
            note: aIngredient.note ?? '',
        };
        if (phase === AdditionalIngredientPhase.FERMENTATION) {
            delete normalized.additionTime;
    ...
    handleAdditionalIngredientChange = (aValue: string, aName: string, aIndex: number) => {
        this.setState((prevState) => {
            ...
            if (["quantity", "additionTime", "triggerValue", "contactTime"].includes(aName)) {
                ...
                additionalIngredientsDTO[aIndex] = aValue === '' ? clearRecipeAction(aStep) : normalizeRecipeAction({...aStep, triggerType: aValue as TriggerType});
            } else if (aName === 'triggerUnit' || aName === 'contactTimeUnit') {
    ...
    validateAdditionalIngredients = (aIngredients: AdditionalIngredientDTO[]): boolean => {
        for (const aIngredient of aIngredients) {
            const aHasId = aIngredient.id !== undefined && aIngredient.id !== null && String(aIngredient.id) !== '';
            if (!aHasId) return false;
            ...
            if (aIngredient.additionTime !== undefined && !(Number(aIngredient.additionTime) > 0)) return false;
    ...
    handleSubmit = (e: FormEvent) => {
        ...
        const missing = (items: Array<{id: string | number}>, masters: Array<{id: string | number}>) =>
            items.filter(item => !masters.some(master => String(master.id) === String(item.id)));
        const missingMalts = missing(maltsDTO, malts);
        const normalizedHops = hopsDTO.map(normalizeHopDto);
        const missingHops = missing(normalizedHops, hops);
        const missingYeasts = missing(yeastsDTO, yeasts);
        const missingAdditional = missing(additionalIngredientsDTO, this.props.additionalIngredients);
        if (missingMalts.length || missingHops.length || missingYeasts.length || missingAdditional.length) {
            this.openValidationDialog('Mindestens eine Rezeptzutat ist nicht mehr in den Stammdaten vorhanden. Bitte ordne alle als unbekannt markierten Zutaten neu zu.');
            return;
        }
        const malts_DTO: MaltDTO[] = maltsDTO.map(malt => ({id: malt.id, quantity: Number(malt.quantity)}));
        if (!normalizedHops.every(validateHopDto)) {
        ...
        const hops_DTO: HopDTO[] = normalizedHops.map(hop => ({...hop, id: hop.id, quantity: Number(hop.quantity)}));
        const yeasts_DTO: YeastDTO[] = yeastsDTO.map(yeast => ({id: yeast.id, quantity: Number(yeast.quantity)}));
    ...
    cancelOrResetForm = () => {
        ...
        this.setState({
            ...
            maltsDTO: selectedBeer.malts.map((malt) => ({id: malt.id, quantity: malt.quantity})),
            hopsDTO: selectedBeer.wortBoiling?.hops.map((hop) => normalizeHopDto(hop)) || [],
            yeastsDTO: selectedBeer.fermentationMaturation?.yeast.map((yeast) => ({id: yeast.id, quantity: yeast.quantity})) || [],
    ...
    addMalts = () => {
        this.setState((prevState) => ({
            maltsDTO: [...prevState.maltsDTO, { id: '', quantity: undefined as any }],
        }));
    }

    addHops = () => {
        this.setState((prevState) => ({
            hopsDTO: [...prevState.hopsDTO, { id: '', quantity: undefined as any, usage: HopUsage.BOIL }],
        }));
    }

    addYeast = () => {
        this.setState((prevState) => ({
            yeastsDTO: [...prevState.yeastsDTO, {id: '', quantity: undefined as any}],
        }));
    }
    ...
    handleBeerSelect = (e: ChangeEvent<HTMLSelectElement>) => {
        ...
        if (selectedBeer) {
            this.setState({
                ...
                cookingTemperatur: selectedBeer.cookingTemperatur || 0,
                fermentationSteps: normalizeMashPlan(selectedBeer.fermentation || []),
                maltsDTO: selectedBeer.malts ? selectedBeer.malts.map(m => ({ id: m.id, quantity: m.quantity })) : [],
                ...
                yeastsDTO: selectedBeer.fermentationMaturation && selectedBeer.fermentationMaturation.yeast ? selectedBeer.fermentationMaturation.yeast.map(y => ({ id: y.id, quantity: y.quantity })) : [],
    ...
    renderCreateBeerForm() {
        ...
        const basicContent = (
            <div className="recipe-form-card"><div className="beer-form-grid">
        ...
        const maltsContent = (
            <>
                <div className="recipe-item-list">{maltsDTO?.map((step, index) => <article className="recipe-item-card" key={index}>
                    <header className="recipe-item-card-header"><strong>{malts.find(malt => String(malt.id) === String(step.id))?.name || 'Neue Malzgabe'}</strong><button type="button" className="cancel-btn brauhaus-button brauhaus-button-danger brauhaus-icon-button" onClick={() => this.removeMalts(index)} title="Malz löschen" aria-label="Malz löschen"><DeleteOutlineIcon fontSize="small" /></button></header>
                    <div className="recipe-item-fields recipe-item-fields--quantity"><label>Malz<select name="id" value={String(step.id)} onChange={(e) => this.handleMaltChange(e.target.value, e.target.name, index)} required={true}><option value="">Malz</option>{!malts.some(malt => String(malt.id) === String(step.id)) && step.id !== '' && <option value={String(step.id)}>Unbekanntes Malz (ID {step.id})</option>}{malts.map((malt) => <option key={malt.id} value={String(malt.id)}>{malt.name}</option>)}</select></label><label>Menge (g)<input type="number" name="quantity" min={0} value={step.quantity} onChange={(e) => this.handleMaltChange(e.target.value, e.target.name, index)} required={true} />{this.renderFieldError(`maltsDTO.${index}.quantity`)}</label></div>
                </article>)}</div>
        ...
        const yeastContent = (
            <>
                <div className="recipe-item-list">{yeastsDTO?.map((step, index) => <article className="recipe-item-card" key={index}>
                    <header className="recipe-item-card-header"><strong>{yeasts.find(yeast => String(yeast.id) === String(step.id))?.name || 'Neue Hefegabe'}</strong><button type="button" className="cancel-btn brauhaus-button brauhaus-button-danger brauhaus-icon-button" onClick={() => this.removeYeast(index)} title="Hefe löschen" aria-label="Hefe löschen"><DeleteOutlineIcon fontSize="small" /></button></header>
                    <div className="recipe-item-fields recipe-item-fields--quantity"><label>Hefe<select name="id" value={String(step.id)} onChange={(e) => this.handleYeastChange(e.target.value, e.target.name, index)} required={true}><option value="">Hefe</option>{!yeasts.some(yeast => String(yeast.id) === String(step.id)) && step.id !== '' && <option value={String(step.id)}>Unbekannte Hefe (ID {step.id})</option>}{yeasts.map((yeast) => <option key={yeast.id} value={String(yeast.id)}>{yeast.name}</option>)}</select></label><label>Menge (g)<input type="number" name="quantity" min={0} value={step.quantity} onChange={(e) => this.handleYeastChange(e.target.value, e.target.name, index)} required={true} />{this.renderFieldError(`yeastsDTO.${index}.quantity`)}</label></div>
                </article>)}</div>
        ...
        const additionalContent = (
            <>
                <div className="recipe-item-list">{additionalIngredientsDTO?.map((aStep, aIndex) => <article className="recipe-item-card additional-ingredient-card" key={aIndex}>
                    <header className="recipe-item-card-header"><strong>{additionalIngredients.find(item => String(item.id) === String(aStep.id))?.name || 'Neue Zutat'}</strong><button type="button" className="cancel-btn brauhaus-button brauhaus-button-danger brauhaus-icon-button" onClick={() => this.removeAdditionalIngredient(aIndex)} title="Zutat löschen" aria-label="Zutat löschen"><DeleteOutlineIcon fontSize="small" /></button></header>
                    <div className="recipe-item-fields additional-ingredient-base">
                        <label>Zutat<select name="id" value={String(aStep.id ?? '')} onChange={(e) => this.handleAdditionalIngredientChange(e.target.value, "id", aIndex)}><option value="">Zutat</option>{!additionalIngredients.some(item => String(item.id) === String(aStep.id)) && aStep.id != null && String(aStep.id) !== '' && <option value={String(aStep.id)}>Unbekannte Zutat (ID {aStep.id})</option>}{additionalIngredients.map((aIngredient) => <option key={aIngredient.id} value={String(aIngredient.id)}>{aIngredient.name}</option>)}</select></label>
                    ...
                    {aStep.phase === AdditionalIngredientPhase.FERMENTATION ? <div className="recipe-field-group"><h4>Gärungsaktion</h4><FermentationActionFields value={aStep} onChange={(value, field) => this.handleAdditionalIngredientChange(value, field, aIndex)} /></div> : <div className="recipe-field-group"><h4>Zugabezeit</h4><div className="recipe-item-fields recipe-item-fields--timing"><label>Zeit<input type="number" min={1} name="additionTime" value={aStep.additionTime ?? ''} onChange={(e) => this.handleAdditionalIngredientChange(e.target.value, "additionTime", aIndex)} /></label><label>Zeiteinheit<select name="timeUnit" value={aStep.timeUnit ?? AdditionalIngredientTimeUnit.DAYS} onChange={(e) => this.handleAdditionalIngredientChange(e.target.value, "timeUnit", aIndex)}><option value={AdditionalIngredientTimeUnit.MINUTES}>Minuten</option><option value={AdditionalIngredientTimeUnit.HOURS}>Stunden</option><option value={AdditionalIngredientTimeUnit.DAYS}>Tage</option></select></label></div></div>}
                    <div className="recipe-field-group"><label>Hinweis<input type="text" name="note" value={aStep.note ?? ''} onChange={(e) => this.handleAdditionalIngredientChange(e.target.value, "note", aIndex)} /></label></div>
                </article>)}</div>
        ...
        return (
            <div className='containerBeerForm'>
                ...
                <RecipeImportDialog
                    ...
                    result={this.props.importResult}
