                    ...
                    <table className="ingredient-table">
                        ...
                        <tbody>{fermentationSteps?.map((step, index) => {
                            ...
                            return <tr key={index} className={this.state.validationErrors[procedureTypeErrorKey] ? 'validation-error-row' : ''}>
                                <td>{isFixed ? <span className="readonly-table-value">{step.type}</span> : <><select key={`${index}-${procedureType}`} aria-label={`Typ ${index + 1}`} aria-invalid={Boolean(this.state.validationErrors[procedureTypeErrorKey])} name="procedureType" value={procedureType} onChange={(e) => this.handleFermentationStepChange(e.target.value, e.target.name, index)}>{procedureTypeOptions.map((option) => <option key={option.value} value={option.value} label={option.label}>{option.label}</option>)}</select>{this.renderFieldError(procedureTypeErrorKey)}</>}</td>
