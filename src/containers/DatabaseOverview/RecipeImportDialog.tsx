    ...
    return <AppDialog open={open} onClose={resetAndCancel} disableClose={busy} title={unresolved.length ? 'Zutaten zuordnen' : 'Rezept importieren'} variant={backendError ? 'error' : busy ? 'progress' : 'info'} className="recipe-import-dialog"
        ...
        {unresolved.length ? <div className="recipe-import-dialog__resolutions" aria-label="Nicht eindeutig zugeordnete Zutaten">
            ...
            {unresolved.map(item => { const key = keyFor(item); const options = resolutionOptions(item, masterData); return <section className="recipe-import-dialog__resolution" key={key}>
                ...
                <div className="recipe-import-dialog__mapping"><FormControl fullWidth size="small"><InputLabel id={`${key}-label`}>Lokale Zuordnung</InputLabel><Select labelId={`${key}-label`} label="Lokale Zuordnung" value={isValidIngredientId(mappings[key]) ? String(mappings[key]) : ''} onChange={event => { if (isValidIngredientId(event.target.value)) { setMappings(current => ({...current, [key]: event.target.value})); setMappingStates(current => ({...current, [key]: 'manual'})); } }}><MenuItem value=""><em>Bitte auswählen</em></MenuItem>{options.map(option => <MenuItem key={String(option.id)} value={String(option.id)}>{option.name}{option.matchType === 'FUZZY' ? ' (ähnlich)' : ''}</MenuItem>)}</Select></FormControl>
